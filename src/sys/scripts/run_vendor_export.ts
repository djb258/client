/*
 * CTB Metadata
 * ctb_id: CTB-7AB9C98BEA4C
 * ctb_branch: sys
 * ctb_path: sys/scripts/run_vendor_export.ts
 * ctb_version: 1.0.0
 * created: 2025-10-23T16:37:00.849529
 * checksum: 416e2948
 */
...
// Fetch eligible enrollments from Neon via MCP
async function getEnrollments(vendorId: string) {
  const res = await fetch(`${MCP_SERVER_URL}/select`, {
    method: "POST",
    headers: {
      "Content-Type": "application/json",
...
async function runExport(vendorId: string, blueprintId: string, vendorTable: string) {
  ...
  const enrollments = await getEnrollments(vendorId);
